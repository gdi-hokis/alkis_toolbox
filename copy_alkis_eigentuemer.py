...
def PrepareCsv(input_csv, output_csv):
...
def SpatialJoinGemFlst( gem, flst):
...
def MakeEigentuemerTable (gem, flst, alkiscsv, prepared_csv,tempgdb, abrufdatum):
    #  CSV wird in fc konvertiert und dann bearbeitet
    PrepareCsv(alkiscsv, prepared_csv)
    ...
    print("FLSTKEY berechnet")
    arcpy.AddField_management("fcalkiscsv", "FSK", "TEXT")
    arcpy.CalculateField_management(
        "eigentuemer",
        "FSK",
        "replaceZerosInFSK(!FKZ![1:-1])",
        "PYTHON3",
...
def DeleteAppendEigentuemerData(aprx_path):
    ...
    print(arcpy.GetMessages())
    print(f"Die Features aus 'fcalkicsv' wurden erfolgreich in 'Eigentuemer_Sichten' angehängt.")
    print("fertig")